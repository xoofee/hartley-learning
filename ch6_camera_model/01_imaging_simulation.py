...
def _plane_box_intersection_polygon(
    n: np.ndarray, d: float, xlim: tuple[float, float], ylim: tuple[float, float], zlim: tuple[float, float]
) -> np.ndarray:
    """
    ...
    y0, y1 = ylim[0], ylim[1]
    z0, z1 = zlim[0], zlim[1]
    # 12 edges of the box (each as (A, B) where A, B are (3,) endpoints)
    corners = np.array([
    ...
    if len(pts) < 3:
        return np.zeros((0, 3))
    pts = np.array(pts)
    # Remove duplicates (within tol) and order into a convex polygon: centroid then sort by angle
    pts = np.unique(pts.round(decimals=9), axis=0)
    if len(pts) < 3:
        return np.zeros((0, 3))
    center = pts.mean(axis=0)
    # Project to plane's 2D: use two in-plane axes
    ...
    for i in range(3):
        ...
        if len(verts) >= 3:
            ax3d.add_collection3d(
                Poly3DCollection([verts], facecolors=colors[i], edgecolors=colors[i], alpha=0.25, linewidths=0.5)
            )
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self) -> None:
        ...
        self.check_show_P_planes = QCheckBox("Show P row planes (3D)")
