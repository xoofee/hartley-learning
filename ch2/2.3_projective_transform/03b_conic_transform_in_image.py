...
def main():
    ...
    # let's transform the conic using the homography matrix
    # conic: x^T C x = 0
    # x' ^T C' x' = 0
    # x ^T H^T C' H x = 0
    # C' = H^(-T) C H^(-1)
    conic_transformed = np.linalg.inv(H.T) @ conic @ np.linalg.inv(H)
    corrected_image = draw_conic(corrected_image, conic_transformed, color=(0, 255, 0), threshold=0.01)
