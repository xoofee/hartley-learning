...
def main():
    ...
    point1 = np.array([h/4, h/4, 1])
    point2 = np.array([h/4*2, h/4*2, 1])
    point3 = np.array([h/4, h/4*3, 1])

    line1 = np.cross(point1, point2)
    line2 = np.cross(point2, point3)

    print(f"line1: {line1}")
    print(f"line2: {line2}")

    image = draw_line(image, line1, color=(0, 0, 255), thickness=2)
    image = draw_line(image, line2, color=(0, 0, 255), thickness=2)
    ...
    # let's transform the lines using the homography matrix
    H_inv_T = np.linalg.inv(H).T
    line1_transformed = H_inv_T @ line1
    line2_transformed = H_inv_T @ line2

    corrected_image = draw_line(corrected_image, line1_transformed, color=(0, 255, 0), thickness=1)
    corrected_image = draw_line(corrected_image, line2_transformed, color=(0, 255, 0), thickness=1)
